...
def gate_center_error_x(y_true, y_pred):
    """Mean average error for gate center prediction.
    ...
    x_loss = tf.math.multiply(tf.cast(p_true, tf.float32), tf.math.abs(cx_pred - cx_true))    #new IROS 2021

    return x_loss

def gate_center_error_y(y_true, y_pred):
    """Mean average error for gate center prediction.
    ...
    y_loss = tf.math.multiply(tf.cast(p_true, tf.float32), tf.math.abs(cy_pred - cy_true))    #new IROS 2021
