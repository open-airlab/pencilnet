...
from networks import ADRNet
from losses import mean_squarred_error, mse_center
...
config['info'] = 'ADRNet -- au_dr and fc and center only output and rmsprop and mae loss and gatenet-output and lr 0.001'
...
# Load the model.
model = ADRNet(config)
logger.set_network(ADRNet)
model.summary()

# Compile the model.
opt = config['optimizer']
model.compile(optimizer=opt, loss=[mse_center],
			  metrics=config['metrics'])
logger.set_loss(mse_center)
logger.save()
